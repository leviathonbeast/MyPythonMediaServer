...
class GcResult:
    ...
    missing_artwork_refs_cleared: int = 0
...
def run_gc(*, vacuum: bool = False) -> GcResult:
    ...
    # ---- 3a. Missing artwork files referenced by the DB ----------------
    # Mirror-image of the orphan-file sweep below: walk every referenced
    # cover_art_id, null out the ones whose file is gone. Without this step
    # an album whose artwork file was deleted (manual rm, failed scan, etc.)
    # keeps a stale reference forever and every getCoverArt request 404s.
    missing_refs_cleared = _cleanup_missing_artwork_refs()

    # ---- 3b. Orphan artwork files on disk ------------------------------
    ...
    result = GcResult(
        ...
        missing_artwork_refs_cleared=missing_refs_cleared,
    ...
    log.info(
        ...
        "stale_refs=%d wal=%s vacuum=%s db=%d→%d bytes",
...
def _cleanup_missing_artwork_refs() -> int:
    """
    Null out `albums.cover_art_id` where the artwork file is missing on disk.

    The inverse of `_cleanup_orphan_artwork`. The two failure modes that
    leave this kind of dangling reference behind:
      - The cache directory was deleted out-of-band (rm -rf, container
        volume mismatch, etc.) but the DB wasn't touched.
      - A scan stored the id but the file write itself failed silently
        (full disk, permissions).

    Without this sweep every album-grid card would render and then 404 on
    getCoverArt forever — there's no other path to clear the column.

    Implementation: read the small set of distinct referenced ids, check
    each on disk via the same resolver getCoverArt uses, then issue a
    single transactional UPDATE for the stale set. We do the disk checks
    outside the transaction because they're slow and read-only.
    """
    referenced = _referenced_cover_art_ids()
    if not referenced:
        return 0

    stale = [cid for cid in referenced if artwork_module.find_artwork_path(cid) is None]
    if not stale:
        return 0
