...
// Grid mode shows the artist's most-recent album cover as a placeholder
// immediately, then upgrades to a real artist photo (Deezer) via
// IntersectionObserver as cards scroll into view. Last.fm no longer ships
// images through their API so the backend prefers Deezer's xl picture.
...
async function upgradeArtistImage(artEl: HTMLElement): Promise<void> {
  ...
  try {
    ...
    // Resolution order:
    //   1. image_cover_art_id → coverArtUrl(): the server has the bytes
    //      cached; the response is one-year immutable so the browser
    //      caches it too. This is the path we want for every artist
    //      once the recovery sweep has run.
    //   2. image_url verbatim: server hasn't cached this artist yet, so
    //      it returned the live Deezer URL. Works but no client cache
    //      and counts against Deezer's CDN.
    //   3. bio.image_url: legacy fallback; null in practice today.
    let url: string | null = null;
    if (detail.image_cover_art_id) {
      url = coverArtUrl(detail.image_cover_art_id, 600);
    } else if (detail.image_url) {
      url = detail.image_url;
    } else if (detail.bio?.image_url) {
      url = detail.bio.image_url;
    }
