...
- **Protocol** — OpenSubsonic 1.16.1 — works with Symfonium, play:Sub,
...
rescans (mtime+size diff); mutagen → ffprobe → filename metadata pipeline;
...
stream-format badge, admin panels for user/folder/scan management.
...
user management; bcrypt-hashed passwords.
...
**Implemented:** browsing (`getMusicFolders`, `getIndexes`,
`getMusicDirectory`, `getAlbum`, `getAlbumList`/`List2`, `getSong`),
search (`search3`), playback (`stream`, `download`, `getCoverArt`),
user management (`getUser`/`getUsers`/`createUser`/`updateUser`/
`deleteUser`/`changePassword`), and the OpenSubsonic extensions endpoint.

**Stubbed** (returns valid empty responses so clients don't error):
playlists, starred, scrobble, now-playing.

**Not yet implemented:** ID3 artist browse (`getArtist`/`getArtists`),
genres, similar-songs, podcasts, internet radio, bookmarks, cross-device
play queue.
...
2. Set `jwt_secret` to a long random string — an empty/guessable secret
   lets anyone forge session tokens.
...
4. Passwords are stored with bcrypt (cost 12); plaintext is never written
   to disk.
5. The web UI stores credentials in `localStorage` so Subsonic calls
...
touched. Coverage: permissions, user CRUD, Subsonic protocol compliance.
